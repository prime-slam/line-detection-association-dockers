...
class Adapter(TensorflowAdapter):
    ...
    def _build_model(self):
        ...
        real_path = get_checkpoint_path(str(self.model_path))
        model = get_predictor(real_path, model)
        return model
    ...
    def __create_descriptors(self, model, lines, image):
        batch_cutouts, batch_heights = self.__create_frame_data(lines, image)
        descriptors, _ = model(batch_cutouts, batch_heights)
        return descriptors

    def __create_frame_data(self, lines, image, cutout_height: int = 100):
        keylines = []
        ...
        for class_id, (x1, y1, x2, y2) in enumerate(lines):
            ...
            keylines.append(kl)
        cutouts = processor.process(image, keylines)
