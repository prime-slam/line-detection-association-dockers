...
class LineTransformer(nn.Module):
    ...
    def preprocess(self, klines_cv, image_shape, pred_superpoint, valid_mask=None):
        ...
        lines = klines_cv.reshape(-1, 2, 2)
        line_lengths = get_line_lengths(lines)
        line_data = {
            "klines": lines,
            "length_klines": line_lengths,
            "angles": get_angles(lines)
        }

        # remove_borders
        _, _, height, width = image_shape
        config = LineTransformer.default_config
        token_distance = config['token_distance']
        max_tokens = config['max_tokens']

        num_klines = len(line_data['klines'])
        if num_klines == 0:
            return line_data

        # line_tokenizer
        line_data = line_tokenizer(line_data, token_distance, max_tokens, pred_superpoint,
                                   image_shape[-2:])
